...
r.post("/", auth(), adminOnly, async (req, res) => {
  ...
  try {
    ...
    emitEvent("category:created", { id: result.insertId, name });
...
r.put("/:id", auth(), adminOnly, async (req, res) => {
  ...
  try {
    ...
    emitEvent("category:updated", { id: Number(req.params.id) });
    res.json({ ok: true });
  } catch (e) {
    if (e.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: "Slug already exists" });
...
r.delete("/:id", auth(), adminOnly, async (req, res) => {
  try {
    ...
    emitEvent("category:deleted", { id: Number(req.params.id) });
    res.json({ ok: true });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: "Server error" });
