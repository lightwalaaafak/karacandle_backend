...
r.get("/sections", async (_, res) => {
  const [rows] = await db.query(
    "SELECT * FROM homepage_sections WHERE is_active=1",
  );
  res.json(rows);
});
...
r.post(
  ...
  async (req, res) => {
    ...
    const [r1] = await db.query(
      "INSERT INTO banners (title, subtitle, video_url, cta_text, cta_link, display_order) VALUES (?,?,?,?,?,?)",
      [title, subtitle, video_url, cta_text, cta_link, display_order || 0],
    );
    emitEvent("cms:banner_created", { id: r1.insertId, title });
...
r.put("/sections/:key", auth(), adminOnly, async (req, res) => {
  ...
  await db.query(
    `INSERT INTO homepage_sections (section_key, heading, body, image_url, meta) VALUES (?,?,?,?,?)
  ...
  emitEvent("cms:section_updated", { section_key: req.params.key });
  res.json({ ok: true });
});
